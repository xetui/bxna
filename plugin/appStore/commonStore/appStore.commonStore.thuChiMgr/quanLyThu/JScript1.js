var quanLyThuFn = {
    urlDefault: function () { return adm.urlDefault + '&act=loadPlug&rqPlug=appStore.commonStore.thuChiMgr.quanLyThu.Class1, appStore.commonStore.thuChiMgr'; },
    urlDefault1: function () { return adm.urlDefault1 + '&act=loadPlug&rqPlug=appStore.commonStore.thuChiMgr.quanLyThu.Class1, appStore.commonStore.thuChiMgr'; },
    setup: function () { },
    loadgrid: function () {
        adm.styleButton();
        var DMID = $('.mdl-head-HangHoaFilterDanhMuc');
        var TuNgay = $('.mdl-head-quanLyThuMdl-TuNgay');
        var DenNgay = $('.mdl-head-quanLyThuMdl-DenNgay');
        var q = $('.mdl-head-search-hangHoaMgr');
        $('#quanLyThuMdl-List').jqGrid({
            url: quanLyThuFn.urlDefault().toString() + '&subAct=get',
            height: '400',
            datatype: 'json',
            loadui: 'disable',
            ...
            sortname: 'TC_NgayTao',
            sortorder: 'asc',
            pager: jQuery('#quanLyThuMdl-Pager'),
            rowNum: 10,
    ...
    refresh: function (grid) {
        if (typeof (grid) == 'undefined') grid == '#quanLyThuMdl-List';
        var timerSearch;
        if (timerSearch) clearTimeout(timerSearch);
        timerSearch = setTimeout(function () {
            $(grid).jqGrid('setGridParam', { url: quanLyThuFn.urlDefault().toString() + '&subAct=get' }).trigger('reloadGrid');
        }, 500);
    },
    edit: function (grid) {
        var s = '';
        ...
        if (s == '') {
            ...
            if (s.indexOf(',') != -1) {
                ...
                //    return false;
                //}
                quanLyThuFn.loadHtml(function () {
                    var newDlg = $('#quanLyThuMdl-dlgNew');
                    $(newDlg).dialog({
                        title: 'Sửa',
                        width: 700,
                        buttons: {
                            'Lưu': function () {
                                quanLyThuFn.save(false, function () {
                                    quanLyThuFn.clearform();
                                    quanLyThuFn.draff(function (dt) {
                        ...
                        open: function () {
                            ...
                            adm.styleButton();
                            quanLyThuFn.clearform();
                            quanLyThuFn.popfn(newDlg);
                            $.ajax({
                                url: quanLyThuFn.urlDefault().toString() + '&subAct=edit',
                                dataType: 'script',
                                data: {
                                    'ID': s
                                },
    ...
    add: function (_newDlg, fn) {
        quanLyThuFn.loadHtml(function () {
            ...
            $(newDlg).dialog({
                ...
                buttons: {
                    'Lưu': function () {
                        quanLyThuFn.save(false, function (_ID) {
                            if (typeof (fn) == 'function') {
                                fn(_ID);
                            }
                            quanLyThuFn.clearform();
                            quanLyThuFn.draff(function (_dt) {
                                var dt = eval(_dt);
    ...
    loc: function () {
        quanLyThuFn.loadHtml(function () {
            var newDlg = $('#quanLyThuMdl-dlgFilter');
            $(newDlg).dialog({
                title: 'Lọc',
                width: 600,
                modal: true,
                buttons: {
                    'Tìm': function () {
                        ...
                        $('#quanLyThuMdl-List').jqGrid('setGridParam', { url: quanLyThuFn.urlDefault().toString() + '&subAct=get&DM_ID=' + _DM_ID + '&KH_ID=' + _KH_ID + '&TuNgay=' + _TuNgay + '&DenNgay=' + _DenNgay }).trigger('reloadGrid');

                    },
                    'Nhập lại': function () {
                        quanLyThuFn.locClearform();
                    },
                    'Đóng': function () {
                        $(newDlg).dialog('close');
                    }
    ...
    del: function (grid) {
        ...
        if (s == '') {
            ...
            if (confirm('Bạn có chắc chắn xóa?')) {// Xác nhận xem có xóa hay không
                $.ajax({
                    url: quanLyThuFn.urlDefault().toString() + '&subAct=del',
                    dataType: 'script',
                    data: { 'ID': s },
                    success: function (dt) {
    ...
    save: function (validate, fn, grid) {
        ...
        $.ajax({
            url: quanLyThuFn.urlDefault().toString(),
            dataType: 'script',
            type: 'POST',
            data: {
                'subAct': 'save',
    ...
    search: function () {
        ...
        if (timerSearch) clearTimeout(timerSearch);
        timerSearch = setTimeout(function () {
            $('#quanLyThuMdl-List').jqGrid('setGridParam', { url: quanLyThuFn.urlDefault().toString() + '&subAct=get&NDTC_ID=' + _DMID + '&q=' + q.val() + '&DenNgay=' + _DenNgay + '&TuNgay=' + _TuNgay }).trigger('reloadGrid');
        }, 500);
    },
    draff: function (fn) {
        $.ajax({
    ...
    createReport: function (id) {
        var request = document.location.href + quanLyThuFn.urlDefault().toString() + '&subAct=reports&ID=' + id;
    ...
    loadHtml: function (fn) {
        var dlg = $('#quanLyThuMdl-dlgNew');
        if ($(dlg).length < 1) {
            adm.loading('Dựng from');
            $.ajax({
                url: '<%=WebResource("appStore.commonStore.thuChiMgr.quanLyThu.htm.htm")%>',
                success: function (dt) {
                    adm.loading(null);
                    $('body').append(dt);
                    if (typeof (fn) == 'function') {
